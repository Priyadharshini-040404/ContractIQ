...
class PostmanCollectionGenerator:
    ...
    def _parse_url(self, path: str, query_params: dict) -> dict:
        ...
        # Replace path parameters with Postman variable syntax
        postman_path = path
        for param in ["pet_id", "order_id"]:
    ...
    def _build_test_script(self, test_case: dict, assertions_map: dict) -> list[str]:
        ...
        # Store dynamic values for chaining
        if test_case.get("type") == "positive" and "POST" in test_case.get("request", {}).get("method", ""):
            lines.append("// Store created resource ID for subsequent tests")
            lines.append("if (pm.response.code === 201) {")
            lines.append("    var jsonData = pm.response.json();")
            lines.append("    if (jsonData.id) {")
            lines.append('        pm.collectionVariables.set("pet_id", jsonData.id);')
            lines.append("    }")
            lines.append("}")
    ...
    def _build_prerequest_script(self, test_case: dict) -> list[str]:
        """Build pre-request script for dynamic test data."""
        lines = []
        request = test_case.get("request", {})
        path = request.get("path", "")

        if "{pet_id}" in path or ":pet_id" in path:
            lines.append('var petId = pm.collectionVariables.get("pet_id");')
            lines.append("if (!petId) {")
            lines.append('    console.warn("pet_id not set - test may fail");')
            lines.append("}")

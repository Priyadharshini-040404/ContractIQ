...
class TestDashboardWithFullData:
    ...
    def test_assertions_tile_falls_back_to_generation_count_with_no_per_test_assertions(
        self, populated_output_dir
    ):
        """The populated_output_dir fixture's execution_results.json
        entries carry no per-test 'assertions' lists, so the tile
        should fall back to the generated-count display rather than
        showing a bogus 0/0."""
        gen = DashboardGenerator(output_dir=str(populated_output_dir))
        data = gen.collect_data()
        stages = gen._pipeline_stages(data)
        tile = next(s for s in stages if s["label"] == "Assertions Generated")
        assert tile["value"] == "51"
        assert tile["sub"] == "positive · negative · edge"

    def test_assertions_tile_shows_pass_fail_breakdown_when_available(self, tmp_path):
        """Group 4, item 14: the 'Assertions Generated' tile must
        reflect assertion RESULTS pulled from execution_results.json's
        per-test assertion list, not just a generation count, whenever
        an execution has actually happened."""
        out = tmp_path / "output"
        out.mkdir()
        (out / "pipeline_result.json").write_text(json.dumps({
            "metadata": {"total_endpoints": 1, "total_test_cases": 2, "total_assertions": 6,
                         "generation_method": "template"},
        }))
        (out / "execution_results.json").write_text(json.dumps({
            "total_tests": 2, "passed": 1, "failed": 1, "pass_rate": "50.0%",
            "results": [
                {"test_id": "a", "assertions": [
                    {"type": "status_code", "passed": True},
                    {"type": "response_time", "passed": True},
                    {"type": "response_body", "passed": True},
                ]},
            ],
            "failures": [
                {"test_id": "b", "assertions": [
                    {"type": "status_code", "passed": False},
                    {"type": "response_time", "passed": True},
                    {"type": "response_body", "passed": False},
                ]},
            ],
        }))
        gen = DashboardGenerator(output_dir=str(out))
        data = gen.collect_data()
        stages = gen._pipeline_stages(data)
        tile = next(s for s in stages if s["label"] == "Assertions Generated")
        # 3 passed (from "a") + 1 passed (from "b") = 4 passed; 2 failed.
        assert tile["value"] == "4/6"
        assert tile["sub"] == "2 failed"

    def test_assertions_tile_all_passed_sub_label(self, tmp_path):
        out = tmp_path / "output"
        out.mkdir()
        (out / "pipeline_result.json").write_text(json.dumps({
            "metadata": {"total_endpoints": 1, "total_test_cases": 1, "total_assertions": 2,
                         "generation_method": "template"},
        }))
        (out / "execution_results.json").write_text(json.dumps({
            "total_tests": 1, "passed": 1, "failed": 0, "pass_rate": "100.0%",
            "results": [{"test_id": "a", "assertions": [
                {"type": "status_code", "passed": True},
                {"type": "response_time", "passed": True},
            ]}],
            "failures": [],
        ...
        stages = gen._pipeline_stages(data)
        tile = next(s for s in stages if s["label"] == "Assertions Generated")
        assert tile["value"] == "2/2"
        assert tile["sub"] == "all passed"

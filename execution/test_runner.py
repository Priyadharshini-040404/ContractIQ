...
class DirectTestRunner:
    ...
    def _run_setup(self, setup: dict) -> str | None:
        """Execute a test case's "setup" block: POST to setup["path"]
        with setup["body"] and return the value of the response field
        named in setup["capture"], or None if creation failed.

        This is what makes destructive (DELETE) tests replayable: each
        run creates its OWN fresh resource to delete instead of
        depending on an ID a previous run already consumed. Entirely
        generic — the resource path/body/capture field all come from
        the test case itself (synthesized by core/test_synthesizer.py),
        never hardcoded here."""
        method = setup.get("method", "POST")
        url = f"{self.base_url}{setup.get('path', '/')}"
        try:
            response = self.session.request(
                method=method,
                url=url,
                headers={"Content-Type": "application/json"},
                json=setup.get("body") or {},
                timeout=10,
            )
            if 200 <= response.status_code < 300:
                body = response.json()
                capture_field = setup.get("capture", "id")
                if capture_field in body:
                    return str(body[capture_field])
    ...
    def _execute_test(self, test_case: dict, stored_ids: dict) -> dict:
        ...
        # Run any "setup" block BEFORE path substitution, so a
        # destructive test case (e.g. DELETE) gets its own disposable
        # resource created fresh on every run, and its "{{token}}"
        # placeholder is filled in with that resource's real ID.
        setup = test_case.get("setup")
        if setup:
            token = self._run_setup(setup)
